...
| 配布物 | 位置づけ | 状態 |
|---|---|---|
| `dist/slide-system-free-v0.2.12.zip` | 無料版Claude向け。特定題材の専用ロジックを配布物から除き、確認・構成・文字量・根拠・条件可視化を汎用化 | ローカル回帰検証済み・無料版Claude実機検証待ち |
...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.12.zip`をアップロードします。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.12`はマラソンを含む特定題材の専用処理を配布物から除外し、汎用の段階納品・構成・根拠・文字量検査へ整理
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版Claudeへ`v0.2.12`を登録し、マラソン以外の提案資料または研修資料でも確認→HTML→PDFの段階フローと品質を実機検証する
