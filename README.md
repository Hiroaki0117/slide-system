...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.9.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。日付付きの運動ロードマップでは、Claudeが短い入力だけを編集し、軽量ビルダーが従来と同じ検証モデルへ展開します。段階表が4行を超える場合は前半・後半へ自動分割します。ページ数は固定せず、反復メニューが短ければ1枚、長ければ概要と項目別詳細へ分割します。主要数値と説明文も異なる文字階層へ分けます。HTML納品時には、距離、練習強度、開始・中止条件を利用者へ確認します。検証に合格できない場合は、実行用ではないことを表示したHTMLを残して停止し、PDF保存を無効にします。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.9`は短いサンプルを10枚のまま維持し、密度が高い週3回メニューだけを概要＋3詳細へ分割して13枚に展開。両方とも静的検証、全ページ画像化、PDFページ一致、フォント、はみ出し検査に合格
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版Claudeへ`v0.2.9`を登録し、実際の入力量に応じたページ分割とHTML納品が上限前に完了するかを実機検証する
