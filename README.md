...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.5.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。次の返信では既存HTMLを変更せずPDFへ高速変換し、PDFを保存してから最低限の確認を行います。HTMLには手動の`PDF保存`ボタンもあります。途中のコード、ページごとの実況、通常のQAログは表示しません。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.5`は確認ゲートに合格したが、承認後の複雑な検証モデル構築で上限へ到達し、成果物未納
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 品質基準を維持したまま入力と構築処理を簡略化する無料版専用の軽量ビルダーを設計する
