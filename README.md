...
| 配布物 | 位置づけ | 状態 |
|---|---|---|
| `dist/slide-system-free-v0.2.3.zip` | 無料版Claude向け。初回内容検査と既存HTML/PDFの修正再開を強化した版 | 再検証待ち |
...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.3.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。次の返信で既存HTMLからPDF化と最終QAを行います。途中のコード、ページごとの実況、通常のQAログは表示しません。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.2`の実機結果を反映し、初回内容検査と既存HTML/PDFの修正再開を強化した`v0.2.3`を作成
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版`v0.2.3`を同じ代表ケースと既存成果物の4経路で再検証
