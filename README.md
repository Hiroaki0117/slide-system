...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.10.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。日付付きの運動ロードマップでは、Claudeが短い入力だけを編集し、軽量ビルダーが従来と同じ検証モデルへ展開します。段階表が4行を超える場合は前半・後半へ自動分割します。ページ数は固定せず、反復メニューが短ければ1枚、長ければ概要と項目別詳細へ分割します。各定例メニューは現状、維持・変更・中止の判断、提案、進め方、理由を分け、現状の再掲だけでは検証に合格しません。HTML納品時には、距離、各メニューの判断と提案、開始・中止条件を利用者へ確認します。検証に合格できない場合は、実行用ではないことを表示したHTMLを残して停止し、PDF保存を無効にします。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.10`は定例メニューを現状・判断・提案・進め方・理由へ分離し、現状を変更案として再掲した場合は静的検証で停止する。短い内容は1枚、詳細が必要な内容は概要＋項目別詳細へ動的分割する
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版Claudeへ`v0.2.10`を登録し、RUNNING_NOTESの定例メニューが現状の再掲ではなく判断付き提案へ変換されるかを実機検証する
