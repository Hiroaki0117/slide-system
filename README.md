...
| 配布物 | 位置づけ | 状態 |
|---|---|---|
| `dist/slide-system-free-v0.2.2.zip` | 無料版Claude向け。段階別の距離・時間目安と進行条件を必須化したHTML先行納品版 | 再検証待ち |
...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.2.zip`をアップロードします。
...
- バージョン: `0.9.0`
- 状態: 無料版`v0.2.1`の実機結果を反映し、段階別負荷、期間計算、練習強度の検査を強化した`v0.2.2`を作成
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版`v0.2.2`を同じ代表ケースで再検証
