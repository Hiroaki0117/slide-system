...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.6.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。日付付きの運動ロードマップでは、Claudeが短い入力だけを編集し、軽量ビルダーが従来と同じ検証モデルと9枚のHTMLへ展開します。検証に合格できない場合も、実行用ではないことを表示したHTMLを残して停止します。次の返信では既存HTMLを変更せずPDFへ高速変換します。HTMLには手動の`PDF保存`ボタンもあります。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.6`の軽量ビルダーは、同じ厳格な静的検証、9枚の画像化、PDF生成、フォント・はみ出し検査に合格。無料版Claude実機検証待ち
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版Claudeへ`v0.2.6`を登録し、未整理入力からHTMLが上限前に返るかを実機検証する
