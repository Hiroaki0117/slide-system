...
2. 無料版では、スキル追加画面から`slide-system-free-v0.2.8.zip`をアップロードします。
...
無料版は、承認後の最初の制作ターンでHTML下書きを納品して必ず停止します。日付付きの運動ロードマップでは、Claudeが短い入力だけを編集し、軽量ビルダーが従来と同じ検証モデルへ展開します。段階表が4行を超える場合は前半・後半へ自動分割します。工程図や要点ページはレイアウト別の文字量上限も検査します。HTML納品時には、距離、練習強度、開始・中止条件を利用者へ確認します。検証に合格できない場合は、実行用ではないことを表示したHTMLを残して停止し、PDF保存を無効にします。
...
- バージョン: `0.9.1`
- 状態: 無料版`v0.2.8`は10枚構成を維持しつつ工程図の文字過多と重複を検査し、検証未完了HTMLのPDF保存・自動変換を禁止。ローカル回帰検証済み
- 既定の出力プロファイル: `html_pdf`
- 既定のコンテンツモード: `standalone`
- 既定のデザインテーマ: `warm_clean`
- 既定のレイアウト方式: `hybrid`
- 次の工程: 無料版Claudeへ`v0.2.8`を登録し、HTML納品、確認ポイント提示、DRAFT時のPDF停止が意図どおり動くかを実機検証する
