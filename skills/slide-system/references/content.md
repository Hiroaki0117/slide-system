...
|---|---|---|
| `explain` | Understand a concept or situation | Context → key idea → mechanism → example → meaning |
| `proposal` | Decide or act | Problem → evidence → options → recommendation → action |
| `report` | Understand a result and response | Conclusion → facts → analysis → implication → response |
| `how_to` | Perform a task | Goal → conditions → steps → cautions → completion check |
| `training` | Learn and practice | Goal → prerequisites → concept → demonstration → exercise → check → reflection |

## Audience

- Beginner: define necessary terms, limit simultaneous concepts, explain why, and use grounded examples.
...
- body: usually 140–220 Japanese characters;
- bullets: normally no more than four;
- one bullet: usually 30–50 Japanese characters.

Guidance for presented slides:
...
- Require a title and date.
- Use the creation date unless an event date, reporting date, or edition date is more meaningful.
